package com.knu.ntttt_server.nft;
...
public class ContractUtils {
    ...
    private boolean useDefaultGasProvider = false;
     ...
     * @return 트랜잭션 해시
     */
    public String sendTransaction(Function function) {
        Transaction transaction = createTransaction(function);
        EthSendTransaction res;
        try {
            res = web3j.ethSendTransaction(transaction).send();
        } catch (IOException e) {
            throw new RuntimeException("Fail to send request");
        }
     ...
     * @param function 호출할 함수
     */
    public List<Type> call(Function function) {
        EthCall callRes;
        Transaction transaction = createTransaction(function);
        try {
            callRes = web3j.ethCall(transaction, DefaultBlockParameter.valueOf("latest")).send();
        } catch (IOException e) {
            throw new RuntimeException("Fail to send request");
        }
        return FunctionReturnDecoder.decode(callRes.getResult(), function.getOutputParameters());
    }

    public TransactionReceipt getReceipt(String transactionHash) {
        EthGetTransactionReceipt ethGetTransactionReceipt;
        try {
             ethGetTransactionReceipt = this.web3j.ethGetTransactionReceipt(transactionHash).send();
        } catch (IOException e) {
            throw new RuntimeException("Fail to send request");
        }
    ...
    @NotNull
    private Transaction createTransaction(Function function) {
    ...
    private BigInteger getNonce() {
        ...
        try {
            ethGetTransactionCount = this.web3j
